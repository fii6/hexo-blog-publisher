...
description: 为 Hexo 博客提供“写作/改稿/本地预览/发布”一体化流程。用户提出写博客、改博客、保存到 Hexo `source/_posts`、本地预览文章、发布文章到远端仓库时使用。执行顺序为：生成或修改 Markdown → 清理 4000 端口并启动本地 Hexo 预览服务 → 等待用户确认 → SSH 连通性检查 → 仅在当次明确授权后执行 git push。
---
...
3. 关键字段：
   - `BLOG_REPO_PATH`：Hexo 仓库本地路径
   - `POSTS_SUBDIR`：文章目录（相对 `BLOG_REPO_PATH`）
   - `PREVIEW_HOST`：本地预览监听地址（默认 `localhost`）
   - `PREVIEW_PORT`：本地预览端口（默认 `4000`）
   - `PREVIEW_LOG_DIR`：预览日志目录（默认 `~/tmp`）
...
1. 先检查 `PREVIEW_PORT`（默认 `4000`）是否被占用；若被占用，先清理占用该端口的进程。
2. 在 `$BLOG_REPO_PATH` 执行：`hexo clean && hexo g && hexo s --ip "$PREVIEW_HOST" --port "$PREVIEW_PORT"`。
3. 成功时返回：`PREVIEW_OK|<url>|<pid>|<log_file>`。
4. 失败时根据返回值处理：`REPO_NOT_FOUND|...`、`NOT_A_GIT_REPO|...`、`PORT_STILL_BUSY|...`、`BUILD_FAIL|...`、`PREVIEW_FAIL|...`。

对用户的默认提示：

- 不再发送 `.md` 预览文件。
- 直接提示用户打开 `http://localhost:4000` 查看预览。
- 附一句简短说明，并明确询问是否推送远端。
- 未收到当次明确“推送/发布/push”指令前，只允许本地修改，不得推送。
...
- 预览服务仅用于本地确认；是否开放给外部设备由用户另行明确说明。
